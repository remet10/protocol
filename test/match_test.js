...
const { newContract, getWeb3, setZdexAmount, getContracts, clone } = require('./utils');
...
contract('Match', async accounts => {
    ...
    // ╚═════════╧══════════╧═══════════════╧═══════════════════════════════════════════════╝
    it('ZDEX discount', async () => {
        await setZdexAmount(zdex, u1, toWei(10000));
        await matchTest({
            baseTokenFilledAmounts: [toWei('1952')],
            users: { u1, u2, u3, relayer },
            baseTokenConfig: {
    ...
    it('eth, taker market order sell, taker full match, maker fee discount', async () => {
        await setZdexAmount(zdex, u2, toWei(10000));
        await matchTest({
